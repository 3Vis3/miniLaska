    ...
    for (r = 0; r < ROWS; r++) {
        for (c = 0; c < COLUMNS; c++) {
            /*if (r < 3 && ((r % 2 == 0 && c % 2 == 0) || (r % 2 != 0 && c % 2 !=0))) { *//*se ci troviamo sulla parte superiore della scacchiera e si presentano le condizioni segnate*//*
                ...
                if(c==2 && r==2) {
                    checkerboard[r * COLUMNS + c].player = PLAYER_1;
                    checkerboard[r * COLUMNS + c].composition[0] = PLAYER_1;
                    checkerboard[r * COLUMNS + c].composition[1] = VOID;
                    checkerboard[r * COLUMNS + c].composition[2] = VOID;
                }
                else if(c==4 && r==4){
                    checkerboard[r * COLUMNS + c].player = PLAYER_2;
                    checkerboard[r * COLUMNS + c].composition[0] = PLAYER_2;
                    checkerboard[r * COLUMNS + c].composition[1] = VOID;
                    checkerboard[r * COLUMNS + c].composition[2] = VOID;
            } else {
...
int capture_check(tower_t *checkerboard, int *turno){
    int r, c;
    char selection; /*variabili char checkerboard da scanf riceve s oppure n e fa il confronto tra selezione e r caratteri n o s*/ /*TODO SISTEMARE LOOP INFINITO SE INSERISCI SN*/
    char si= 's';
...
bool vittoria(tower_t *checkerboard, int *turn) {
    int y, x, counter=0;
    if(!capture_check(checkerboard,turn)) {
        return 0;
    }
    for(y = 0; y < ROWS; y++){
        for(x = 0; x<COLUMNS; x++){
...
int main() {
    ...
    while (!vittoria(&(checkerboard[0][0]), &turn)) {
        if (capture_check(&checkerboard[0][0], &turn) == 0) {
            printf("\nTURNO GIOCATORE %d \n", turn);
            ...
            if (piece_selection(&(checkerboard[0][0]), x, y, turn)) {
                ...
                move_selection(&(checkerboard[0][0]), x, y, turn, move_x,move_y); /*se la mossa è valida stampa chessboard*/
                turn_update(&turn);
            }
