...
int main() {
    ...
    while (!win(&(checkerboard[0][0]), &turn)) {
        if (!capture_check(&checkerboard[0][0], &turn)) {
            printf("\nTURNO GIOCATORE %d \n", turn);
            printf("Seleziona le coordinate della pedina x , y: \n");
            scanf("%d %d", &x,
                  &y); /*inserimento pedina da controllare*/ /*TODO, RICHIEDERE SELEZIONE SE INSERISCI non numeri*/
            ...
            checkerboard_print(&(checkerboard[0][0]));
            turn_update(&turn);
        } else if (capture_check(&checkerboard[0][0], &turn) == 2) {
            continue;
        }
    }
    turn_update(&turn);
